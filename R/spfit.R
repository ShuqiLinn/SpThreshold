...
#'      \code{area}. An intercept is included automatically via the formula
#'      (e.g., \code{y ~ x} fits an intercept and a slope on \code{x}). To
#'      fit without an intercept, write \code{y ~ x + 0} or \code{y ~ x - 1}.
#'    \item \strong{Vector interface:} pass \code{y}, \code{X}, and \code{loc}.
#'      The intercept is \emph{not} added automatically; include a column of
#'      1s in \code{X} (e.g., \code{cbind(1, x)}) if an intercept is desired.
#' }
...
spfit <- function(data = NULL,
   ...
   ## Determine which interface is in use
   ...
   ## Track which interface was used (for the intercept warning below)
   used_vector_interface <- all_vec
   
   ## Translate data frame interface to vectors
   if(all_df){
   ...
   ## Soft check: warn if the vector interface user forgot the intercept
   if(used_vector_interface && !all(X[, 1] == 1)){
      warning("First column of X is not all 1s. If you intended to include ",
              "an intercept, use cbind(1, X). spfit() does not add an ",
              "intercept automatically when using the vector interface; ",
              "if you intended to fit without one, you can ignore this ",
              "warning.",
              call. = FALSE)
   }
   
   ## Validate W (coerce sparse Matrix first; sparse objects don't pass is.numeric)
   ...
   if(min(loc_unique) < 1L || max(loc_unique) > n){
      stop(sprintf("loc values must be integers in 1:%d (= nrow(W)). ", n),
           sprintf("Got range [%d, %d].", min(loc), max(loc)))
   }
   ...
   ## Validate model_indicator, mcmc_samples, burnin
   ...
   ## Validate optional priors and initial values
   ...
   ## C++ engine expects 0-indexed loc
